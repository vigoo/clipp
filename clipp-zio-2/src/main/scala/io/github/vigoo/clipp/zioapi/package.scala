...
package object zioapi {
  ...
  def parametersFromArgs[T: Tag : IsNotIntersection](spec: Parameter.Spec[T]): ZLayer[Console & ZIOAppArgs, ParserFailure, T] =
    (for {
      args <- ZIO.service[ZIOAppArgs]
      result <- Clipp.parseOrFail(args.getArgs, spec)
    } yield result).toLayer

  def effectfulParametersFromArgs[R, T: Tag: IsNotIntersection](createSpec: ZioDSL[R] => Parameter.Spec[T]): ZLayer[Console & ZIOAppArgs & R, ParserFailure, T] = {
    for {
